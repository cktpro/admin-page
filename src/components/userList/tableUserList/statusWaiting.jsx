...
import { actionGetAllUsersWaiting } from "users/Users/getUserListWaiting/action";
...
function StatusWaiting() {
  ...
  // declare resGetAllUserWaiting?.payload of userWaitingReducer state
  const resGetAllUserWaiting = useSelector(
    (state) => state.userWaitingReducer
  );
  ...
  const [userListWaiting, setUserListWaiting] = useState([]);
  ...
  const getAllUser = useCallback(() => {
    dispatch(actionGetAllUsersWaiting(condition));
  ...
  useEffect(() => {
    setUserListWaiting(resGetAllUserWaiting?.payload);

    // console.log('««««« userListWaiting »»»»»', userListWaiting);

    setCondition((prev) => ({
      ...prev,
      total: resGetAllUserWaiting?.payload?.total || defaultPagination.total,
      page: resGetAllUserWaiting?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllUserWaiting?.payload?.pageSize || defaultPagination.pageSize,
    }));
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      userListWaiting?.payload?.forEach((item, index) => {
  ...
  return resGetAllUserWaiting?.isLoading ? (
    ...
    <>
      <div className="cover_table_userlist">
        <Table
          ...
          dataSource={userListWaiting?.payload}
          pagination={false}
        />
      </div>
