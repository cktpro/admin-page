...
import { actionGetAllUsersCanceled } from "users/Users/getUserListCanceled/action";
...
function StatusCanceled() {
  ...
  // declare resGetAllUserCanceled?.payload of userCanceledReducer state
  const resGetAllUserCanceled = useSelector(
    (state) => state.userCanceledReducer
  );
  ...
  const [userListCanceled, setUserListCanceled] = useState([]);
  ...
  const getAllUsers = useCallback(() => {
    dispatch(actionGetAllUsersCanceled(condition));
  ...
  useEffect(() => {
    setUserListCanceled(resGetAllUserCanceled?.payload);

    setCondition((prev) => ({
      ...prev,
      total: resGetAllUserCanceled?.payload?.total || defaultPagination.total,
      page: resGetAllUserCanceled?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllUserCanceled?.payload?.pageSize ||
    ...
    resGetAllUserCanceled?.payload,
  ]);

  // handle user changed page of table antd
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      userListCanceled?.payload?.forEach((item, index) => {
  ...
  return resGetAllUserCanceled?.isLoading ? (
    ...
    <>
      <div className="cover_table_userlist">
        <Table
          ...
          dataSource={userListCanceled?.payload}
          pagination={false}
        />
      </div>
