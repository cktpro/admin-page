...
import { actionGetAllUsersCompleted } from "users/Users/getUserListCompleted/action";
...
function StatusCompleted() {
  ...
  // declare resGetAllUserCompleted?.payload of UserCompletedReducer state
  const resGetAllUserCompleted = useSelector(
    (state) => state.userCompletedReducer
  );
  ...
  const [userListCompleted, setUserListCompleted] = useState([]);
  ...
  const getAllUser = useCallback(() => {
    dispatch(actionGetAllUsersCompleted(condition));
  ...
  useEffect(() => {
    setUserListCompleted(resGetAllUserCompleted?.payload);

    // console.log('««««« userListCompleted »»»»»', userListCompleted);

    setCondition((prev) => ({
      ...prev,
      total:
        resGetAllUserCompleted?.payload?.total || defaultPagination.total,
      page: resGetAllUserCompleted?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllUserCompleted?.payload?.pageSize ||
    ...
    resGetAllUserCompleted?.payload,
  ]);
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      userListCompleted?.payload?.forEach((item, index) => {
  ...
  return resGetAllUserCompleted?.isLoading ? (
    ...
    <>
      <div className="cover_table_userlist">
        <Table
          ...
          dataSource={userListCompleted?.payload}
          pagination={false}
        />
      </div>
