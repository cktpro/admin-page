...
import { actionGetAllOrdersPaid } from "store/Orders/getOrderListPaid/action";
...
function StatusPaid() {
  ...
  // declare resGetAllOrdersPaid?.payload of orderPaidReducer state
  const resGetAllOrdersPaid = useSelector(
    (state) => state.orderPaidReducer
  );
  ...
  const [ordersListPaid, setOrdersListPaid] = useState([]);
  ...
  const getAllOrders = useCallback(() => {
    dispatch(actionGetAllOrdersPaid(condition));
  ...
  useEffect(() => {
    setOrdersListPaid(resGetAllOrdersPaid?.payload);

    // console.log('««««« ordersListPaid »»»»»', ordersListPaid);

    setCondition((prev) => ({
      ...prev,
      total: resGetAllOrdersPaid?.payload?.total || defaultPagination.total,
      page: resGetAllOrdersPaid?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllOrdersPaid?.payload?.pageSize || defaultPagination.pageSize,
    }));
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      ordersListPaid?.payload?.forEach((item, index) => {
  ...
  return resGetAllOrdersPaid?.isLoading ? (
    ...
    <>
      <div className="cover_table_orderlist">
        <Table
          ...
          dataSource={ordersListPaid?.payload}
          pagination={false}
        />
      </div>
