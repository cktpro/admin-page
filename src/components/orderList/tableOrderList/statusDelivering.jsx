...
import { actionGetAllOrdersDelivering } from "store/Orders/getOrderListDelivering/action";
...
function StatusDelivering() {
  ...
  // declare resGetAllOrdersDelivering?.payload of orderDeliveringReducer state
  const resGetAllOrdersDelivering = useSelector(
    (state) => state.orderDeliveringReducer
  );
  ...
  const [ordersListDelivering, setOrdersListDelivering] = useState([]);
  ...
  const getAllOrders = useCallback(() => {
    dispatch(actionGetAllOrdersDelivering(condition));
  ...
  useEffect(() => {
    setOrdersListDelivering(resGetAllOrdersDelivering?.payload);

    // console.log('««««« ordersListDelivering »»»»»', ordersListDelivering);

    setCondition((prev) => ({
      ...prev,
      total:
        resGetAllOrdersDelivering?.payload?.total || defaultPagination.total,
      page: resGetAllOrdersDelivering?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllOrdersDelivering?.payload?.pageSize ||
    ...
    resGetAllOrdersDelivering?.payload,
  ]);
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      ordersListDelivering?.payload?.forEach((item, index) => {
  ...
  return resGetAllOrdersDelivering?.isLoading ? (
    ...
    <>
      <div className="cover_table_orderlist">
        <Table
          ...
          dataSource={ordersListDelivering?.payload}
          pagination={false}
        />
      </div>
