...
function StatusDelivering() {
  ...
  // declare resGetAllOrdersDelivering of orderDeliveringReducer state
  const resGetAllOrdersDelivering = useSelector(
    (state) => state.orderDeliveringReducer.payload
  );

  // declare defaultPagination of get all order completed
  const defaultPagination = {
    total: resGetAllOrdersDelivering?.total || 0,
    page: resGetAllOrdersDelivering?.page || 1,
    pageSize: resGetAllOrdersDelivering?.pageSize || 10,
  };
  ...
  useEffect(() => {
    setOrdersListDelivering(resGetAllOrdersDelivering);

    // console.log('««««« ordersListDelivering »»»»»', ordersListDelivering);

    setCondition((prev) => ({
      ...prev,
      total: resGetAllOrdersDelivering?.total,
      page: resGetAllOrdersDelivering?.page,
      pageSize: resGetAllOrdersDelivering?.pageSize,
    }));

    // window.scrollTo(0, 0);
  }, [resGetAllOrdersDelivering]);
