...
function StatusCanceled() {
  ...
  // declare resGetAllOrdersCanceled?.payload of orderCanceledReducer state
  const resGetAllOrdersCanceled = useSelector(
    (state) => state.orderCanceledReducer
  );
  ...
  useEffect(() => {
    setOrdersListCanceled(resGetAllOrdersCanceled?.payload);

    setCondition((prev) => ({
      ...prev,
      total: resGetAllOrdersCanceled?.payload?.total || defaultPagination.total,
      page: resGetAllOrdersCanceled?.payload?.page || defaultPagination.page,
      pageSize:
        resGetAllOrdersCanceled?.payload?.pageSize ||
    ...
    resGetAllOrdersCanceled?.payload,
  ]);
  ...
  useEffect(() => {
    const filterCustomer = () => {
      ...
      ordersListCanceled?.payload?.forEach((item, index) => {
  ...
  return resGetAllOrdersCanceled?.isLoading ? (
